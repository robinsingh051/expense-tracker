    ...
    if (confirm("Are You Sure?")) {
      try {
        ...
        const response = await axios.delete(
          `http://localhost:4000/expense/${li.id}`
        );
...
async function editItem(e) {
  if (e.target.classList.contains("edit")) {
    if (confirm("Are You Sure?")) {
      try {
        const li = e.target.parentElement;
        const res = await axios.get(`http://localhost:4000/expense/${li.id}`);
...
//   try {
//     const response = await axios.get("http://localhost:4000/users/getfiles");
